...
#define nrn_init _nrn_init__dop_ExpSynSTDP
#define _nrn_initial _nrn_initial__dop_ExpSynSTDP
#define nrn_cur _nrn_cur__dop_ExpSynSTDP
#define _nrn_current _nrn_current__dop_ExpSynSTDP
#define nrn_jacob _nrn_jacob__dop_ExpSynSTDP
#define nrn_state _nrn_state__dop_ExpSynSTDP
#define _net_receive _net_receive__dop_ExpSynSTDP 
#define state state__dop_ExpSynSTDP 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 add_nrn_fornetcons(_mechtype, _fnc_index);
 	...
 	ivoc_help("help ?1 dop_ExpSynSTDP /home/marcin/WORKPLACE/Python/NeuralSimulation/NEURON/TheCreatureProject/x86_64/dop_ExpSynSTDP.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t;   if (_lflag == 1. ) {*(_tqitem) = 0;}
 {
   if ( _lflag  == 0.0 ) {
    ...
    double __state = g;
    double __primary = (g + _args[0] * ( 1.0 + _args[1] ) * ( 1.0 + dop )) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau ) - __primary );
    g += __primary;
  } else {
 g = g + _args[0] * ( 1.0 + _args[1] ) * ( 1.0 + dop ) ;
       }
 _args[2] = t ;
