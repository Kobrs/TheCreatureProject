...
class Creature(object):
    ...
    def control_track_binary(self, spikes):
        ...
        if trackL_activated:
            self._log("moving trackL")
            self.body.step_trackL()
        if trackR_activated:
            self._log("moving trackR")
    ...
    def step(self):
        ...
        # self._log("creature spike len=%d for creature"%np.size(spikes), self)
        ...
        # self._log("NEURON sim duration:", time.clock() - begin)
        self._log("Simulated time: %f"%h.t)
    ...
    def plot_activity(self):
        ...
        for rvec in self.net.record_vectors_dict.values():
            ...
            self._log(np.shape(y))
            # self._log(np.shape(rvec[1].as_numpy()))
            # self._log(rvec[1].as_numpy())
            # raise SystemExit
