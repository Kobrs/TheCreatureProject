...
print "Using follwoing random generator seed:", seed
np.random.seed(seed)
plt.ion()
...
DNA = create_specimen(len_min=20000, len_max=100000)
...
creature = Creature.Creature(display_sim=display_sim, use_clock=False,
...
# creature.setup_plotting(time_window=100)
# """
while True:
    print "stepping"
    creature.step()
    ...
    for i in xrange(num_of_regions):
        if im.conf['dopamine-r%d'%i] is True:
            ...
            creature.stimulate_pleasure(region=i, dop=100, dopdr=0.04)
            im.set_state("dopamine-r%d"%i, False)
        
        # add dop and dopdr config to the gui - float support


        # FIXME: I suspect that reaction of netowrk to dopamine may be due to 
        #         a spiking after sim_init call rather that to dopmaine itself - test it


    # creature.plot_activity()
