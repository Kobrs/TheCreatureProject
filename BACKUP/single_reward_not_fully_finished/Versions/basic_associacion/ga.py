...
class Specimen(GAinterpreter):
    ...
    def fitness_score(self, stimuli, output_cells, reflex=20, interia=5, sim_time=None):
        ...
        while t<=sim_time:
            ...
            # This is kinda different than real beahvior: proper reaction to
            #   stimuli on cell0 is going 
            ...
            for i, timer in enumerate(reflex_timers):
                if round(timer, 5) > 0:
                    ...
                    if (not mov[0]) and mov[1] is True and i==0:
                        score +=0.05

                    # Reward it for moving forward
                    elif mov[0] and (not mov[1]) is True and i==1:
                        score +=0.05

                else:
                    # Reset timer
                    reflex_timers[i] = 0
                    if mov[0] or mov[1] is True:
                        score -= 0.01
