...
class Specimen(GAinterpreter):
    ...
    def fitness_score(self, stimuli, reflex=20, interia=5):
        ...
        mov_record = []
        timers_record = []
        mov = [False, False]  # index 0 is for left, 1 for right
        mov_timers = [0, 0]  # timers to measure movement elapsed time
        dt = round(1. / h.steps_per_ms, 4)
        t = 0
        while t<=sim_time:
            ...
            # Make it move forward at all times
            if (mov[0] ^ mov[1]) is True:  # xor them
                score += 0.001
        ...
        # plt.figure(2)
        # plt.plot(timeline, np.array(mov_record)[:, 0])
        # plt.plot(timeline, np.array(timers_record)[:, 0])  # plot timers
        # plt.plot(timeline, np.array(mov_record)[:, 1])
        # plt.plot(timeline, np.array(timers_record)[:, 1])  # plot timers
        # plt.figure(3)
