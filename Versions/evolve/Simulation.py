...
class Environment(object):
    ...
    def _spawn_target(self, x, y, radius=10, light_source=False):
        ...
        if light_source:
            ...
            # assign id so we can delete it from light_sources list when picked up
            shape.lsource_id = len(self.light_sources)
        else:
    ...
    def spawn_light_sources(self, n=1):
        # Simplified wrapper for spawn_targets
        self.spawn_targets([self.WIDTH, self.HEIGHT], radius=15,
    ...
    def kill_creature(self, i):
        # Get creature body and shapes
        creature = self.creatures[i]
        body = creature.body.creature
    ...
    def step_sim(self):
        ...
        # teleport to other opposite wall if it teached wall
        if body.creature.position[0] > self.WIDTH:
            body.creature.position = (0, body.creature.position[1])
        if body.creature.position[0] < 0:
            body.creature.position = (self.WIDTH, body.creature.position[1])
        if body.creature.position[1] > self.HEIGHT:
            body.creature.position = (body.creature.position[0], 0)
        if body.creature.position[1] < 0:
