...
class GAinterpreter(object):
    ...
    def decode(self, DNA):
        ...
        #   them separately.
        dop = b.b_to_int(DNA[0:10])
        dopdr = b.b_to_int(DNA[10:26])
        self.DNA = DNA[26:]
...
class Specimen(GAinterpreter):
    def __init__(self, DNA, cell, cells_per_pain_stimuli=3, num_of_pain_stimuli=2,
                 interpreter=None,frame_len=6, graph=False):
        ...
        * manual, which allows plotting and graphing:
        ...
        self.num_of_pain_stimuli = num_of_pain_stimuli
        self.graph = graph
        self.dop = dop
        self.dopdr = dopdr
    ...
    def release_dopamine(self, region):
        ...
        for cell in self.cells_dict.values():
            # We divide brain area to five segments 51 units each
            if int(cell.x / 51) == region:
                # Find all connections from this cell
                cell.dopamine_reaction(self.dop, self.dopdr)
