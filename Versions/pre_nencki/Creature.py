...
class Creature(object):
    def __init__(self, display_sim=False, clock=0, creature_sim_dt=1./40,
        ...
        quickest_run = 10
        self.body = Body_sim.Simulation(engine_force=0.35, sensor_h=200,
            sensor_a=40, num_of_targets=num_of_targets, WIDTH=1920, HEIGHT=1080,
            clock=clock, display=display_sim)
    ...
    def apply_random_stim(self, n=5):
        # FIXME: Even though this stimuli is applied, the resulting stimulation of motor cells doesn't lead to movemnt
    ...
    def step(self):
        ...
        for src_id, spikes in spikes.iteritems():
            ...
            if len(spikes) != 0:
                hist_data.append((self.net.cells_dict[src_id].x, len(spikes)))
