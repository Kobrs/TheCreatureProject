...
class Creature(object):
    def __init__(self, display_sim=False, clock=0, creature_sim_dt=1./40,
                 creature_lifetime=0, sensor_cells=[1, 2, 3, 4, 5],
                 trackL_cells=[32, 33, 34, 35], trackR_cells=[64, 65, 66, 67],
                 num_of_targets=10, num_lsources=0, engine_force=0.15,
                 creature_spawn_loc=None, WIDTH=1920, HEIGHT=1080,
        ...
        :param clock: set pygame clock value to maintain steady framerate
        ...
        quickest_run = 10
        self.body = Body_sim.Simulation(engine_force=self.engine_force, sensor_h=200,
            sensor_a=40, num_of_targets=num_of_targets, WIDTH=WIDTH, HEIGHT=HEIGHT,
            clock=clock, num_lsources=num_lsources, display=display_sim,
            creature_spawn_loc=creature_spawn_loc, use_sensor=use_sensor)
    ...
    def remove_light_source(self, i):
        self.body.remove_light_source(i)


    def spawn_light_sources(self, n=1):
        self.body.spawn_targets([self.WIDTH, self.HEIGHT], radius=15,
